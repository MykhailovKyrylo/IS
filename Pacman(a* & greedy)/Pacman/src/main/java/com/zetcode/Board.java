...
//    private boolean dying = false;
    ...
    private int getEvcridDistance(Pair<Integer, Integer> pos0, Pair<Integer, Integer> pos1) {
        return sqr(pos0.getFirst() - pos1.getFirst()) + sqr(pos0.getSecond() - pos1.getSecond());
    }

    private void runPacmanBFS(Pair<Integer, Integer> start_position, Pair<Integer, Integer> final_position) {
        ...
        HashMap<Pair<Integer, Integer>, Direction> reachable = new HashMap<>();
        for (Pair<Pair<Integer, Integer>, Direction> pos_dir : getNeightboursCells(final_position)) {
            if (!isAvailableCell(pos_dir.getFirst())) continue;
            reachable.put(pos_dir.getFirst(), getOppositeDirection(pos_dir.getSecond()));
        }

        while (!reachable.isEmpty()) {
            Pair<Integer, Integer> next_position = new Pair<>(0, 0);
            int min_evclid_distance = 10000000;
            for (Pair<Integer, Integer> position : reachable.keySet()) {
                int evclid_distance = getEvcridDistance(position, start_position);
                if (evclid_distance < min_evclid_distance) {
                    min_evclid_distance = evclid_distance;
                    next_position = position;
                }
            }

            Pair<Pair<Integer, Integer>, Direction> next_pos_dir = new Pair<>(next_position, reachable.get(next_position));
            dx_dy.put(next_pos_dir.getFirst(), next_pos_dir.getSecond());
            reachable.remove(next_position);

            for (Pair<Pair<Integer, Integer>, Direction> pos_dir : getNeightboursCells(next_position)) {
                Pair<Integer, Integer> pos = pos_dir.getFirst();
                if (!isAvailableCell(pos)) continue;
                if (dx_dy.containsKey(pos)) continue;
    ...
    private void runPacmanGreedy(Pair<Integer, Integer> final_position) {
        screenData[convertToRawIndex(final_position)] = (short) (screenData[convertToRawIndex(final_position)] | 32);
        ...
        while (!reachable.isEmpty()) {
            Pair<Integer, Integer> next_position = new Pair<>(0, 0);
            int index = getRandomTo(reachable.size());
            int i = 0;
            for (Pair<Integer, Integer> position : reachable.keySet()) {
                next_position = position;
                if (i == index) {
    ...
    private int getRandomTo(int to) {
        int random = (int) (Math.random() * to);
        return random;
    }

    private Pair<Integer, Integer> getRandomDestinyPosition() {
        int x = getRandomTo(N_BLOCKS);
        int y = getRandomTo(N_BLOCKS);
    ...
    private void initLevel() {
        ...
        switch (flag) {
            case 1:
                Pair<Integer, Integer> current_position = new Pair<>(pacman_x / BLOCK_SIZE, pacman_y / BLOCK_SIZE);
                runPacmanBFS(current_position, getRandomDestinyPosition());
                break;
            case 2:
                runPacmanGreedy(getRandomDestinyPosition());
                break;
        }
    }
